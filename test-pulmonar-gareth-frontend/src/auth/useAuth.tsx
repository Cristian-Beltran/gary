...
const normalizeUserType = (type: string | null | undefined): UserType | null => {
  if (!type) return null;
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  login: async (credentials) => {
    ...
    try {
      ...
      set({
        user,
        token: access_token,
        type: normalizeUserType(user.type) as UserType,
        isLoading: false,
        version: true,
      });
    } catch (err) {
  ...
  loadFromStorage: () => {
    ...
    const type = normalizeUserType(Cookies.get("auth_type"));
  ...
  verifyToken: async () => {
    try {
      ...
      set({ user, token, type: normalizeUserType(user.type), isLoading: false });
    } catch (err) {
      set({ isLoading: false });
      throw err;
    }
